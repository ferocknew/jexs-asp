...
<% 
	...
	dim searchid
	searchid=J.NumberYn(request.QueryString("searchid"))
%>
...
<!--
	...
	function EditBindSystem(sNindex,sLindex,oLindex,bvalue){
		var sN=doc.getElementById("systemName");
		var sL=doc.getElementById("systemList");
		var oL=doc.getElementById("objectList");
		var bL=doc.getElementById("bindList");
		if(sN && sL && oL && bL){
			var optionValue=bvalue;
			var optionText="[" + OptionTextByValue(sN,sNindex) + "]" + SystemListRev(sNindex,sLindex) + " ∞ " + OptionTextByValue(oL,oLindex);
			if(BindCheck(optionValue)) bL.options[bL.length]=new Option(optionText,optionValue);
			SameSystem();
		}
	}
	function OptionTextByValue(o,v){
		for(oi=0;oi<o.length;oi++){
			if(o.options[oi].value==v) return o.options[oi].text;
		}
	}
...
<% 
S.MainOpen
if request.querystring("action")="editsearch" then
	...
	S.EditSearch searchid,searchName,bindList,objectListElement,systemListElement,stylesheet
	
	S.MainClose
	J.close
	
	response.redirect "../../" & SameManagePath & "Inc/Action.asp?action=editsearch2js&searchid=" & searchid
	response.end
end if
	
		
dim searchinfo
searchinfo=S.Main.data.GetRows("select * from search where id=" & searchid)

dim searchobject
searchobject=S.Main.data.GetRows("select * from searchobject where searchid=" & searchid)
%>


	<div class="app_title">编辑搜索引擎</div>
	
  <div class="list_box">
  <form name="form1" method="post" action="?action=editsearch&searchid=<%= searchid %>">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td><fieldset><legend>搜索引擎名称</legend>
            <input type="text" name="searchName" value="<%= searchinfo(1,0) %>">
            </fieldset></td>
      </tr>
            ...
            <table border="0" cellspacing="6" cellpadding="0">
              <tr align="center"> 
                ...
                <td width="140" rowspan="2"><p> 
                    <select name="objectList">
					  ...
					  <% 
					  dim ii
					  for ii=0 to ubound(searchobject,2)
					  	response.write "<option value=""" & searchobject(2,ii) & """>" & searchobject(3,ii) & "</option>"
					  next
					  %>
                    </select>
                  </p>
              ...
              <tr align="center">
				...
				<!--
					...
					function SystemListRev(menuid,rageid){
						var sE=systemElements;
						var sL=doc.getElementById("systemList");
						if(parseInt(menuid)>0){
							if(sE){
                ...
                <td>默认搜索对象： 
                  <select name="objectListElement">
                    <option value="0" selected>所有对象</option>
					  <% 
					  for ii=0 to ubound(searchobject,2)
					  	if searchinfo(3,0)=searchobject(2,ii) then
							response.write "<option value=""" & searchobject(2,ii) & """ selected>" & searchobject(3,ii) & "</option>"
						else
							response.write "<option value=""" & searchobject(2,ii) & """>" & searchobject(3,ii) & "</option>"
						end if
					  next
      ...
      <tr>
        <td><fieldset>
            ...
            <textarea name="stylesheet" id="stylesheet" rows="6" style="width:100%;"><%= S.Main.Encode(searchinfo(2,0)) %></textarea>
			</fieldset></td>
      </tr>
      <tr><td height=10></td></tr>
      <tr>
        <td align="center">
            <input type="submit" name="Submit" value="保存搜索引擎　生成调用代码">
        </td>
      </tr>
    </table>
  </form>
...
<% 
dim searchrange
searchrange=S.Main.data.GetRows("select * from searchrange where searchid=" & searchid)
response.write "<script language=""JavaScript"">" & vbcrlf
for ii=0 to ubound(searchrange,2)
	response.write "EditBindSystem(" & searchrange(2,ii) & "," & searchrange(3,ii) & "," & searchrange(4,ii) & ",'" & searchrange(2,ii) & "|" & searchrange(3,ii) & "|" & searchrange(5,ii) & "|" & searchrange(4,ii) & "');" & vbcrlf
next
if searchinfo(4,0)>0 then response.write "doc.getElementById('systemListElement').value=" & searchinfo(4,0) & ";" & vbcrlf
