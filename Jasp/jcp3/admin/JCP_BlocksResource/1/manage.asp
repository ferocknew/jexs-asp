		...
		if instr(t_link,"_link")>0 then
			if len(rslist(ubound(rslist,1),i))>8 then
				...
				if TemplateType=4 then
					...
					t_html=replace(t_html,"<link ","<a href=""../" & t_tableid & "," & replace(t_classid,"&",",") & "/Content" & rslist(ubound(rslist,1)-1,i) & "." & J.FileExt & """ ")
				end if
			end if
		else
			if TemplateType=4 then
				...
				t_html=replace(t_html,"<link ","<a href=""../" & t_tableid & "," & replace(t_classid,"&",",") & "/Content" & rslist(ubound(rslist,1),i) & "." & J.FileExt & """ ")
			end if
		end if
