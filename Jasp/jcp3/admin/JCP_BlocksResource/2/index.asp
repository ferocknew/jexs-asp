...
<form name="blockform" method="post" action="action.asp?action=createblock" style="margin:0;">
    <table width="100%" height="101" border="0" cellpadding="4" cellspacing="0">
      <tr> 
        ...
        <td colspan="2"><input type=text id=blockname name=blockname value="限数文字列表JS版" onblur="IllegalCheck(this.value,this);BlankCheck(this.value,this);"> 
        </td>
      </tr>
      <tr> 
        <td height="29" align="right">模块说明：</td>
        <td colspan="2"><input type=text id=blockexplain name=blockexplain value="用于类似TOP10.js短列表JS文件，供多页面调用" onblur="IllegalCheck(this.value,this);BlankCheck(this.value,this);" style="width:100%;"></td>
      </tr>
      <tr> 
      ...
      <tr> 
        ...
        <td colspan="2"><input type="radio" name="ClassFrom" id="ClassFrom" class="select" onclick="ClassStateSet(1);" value="1" disabled>
          来自模板 
          <input type="radio" name="ClassFrom" id="ClassFrom" class="select" onclick="ClassStateSet(2);" value="2" checked>
          自定义 　 
      ...
      <tr> 
        ...
        <td><select name="TableItemList" id="TableItemList" onchange="if(this.selectedIndex==0){ReplaceString.value='';}else{ReplaceString.value=this.options[this.selectedIndex].text;ReplaceString.select();}">
            ...
            <option value="uptime">发布时间</option>
            <option value="hits">点击量</option>
          </select> <input type="button" class="system_button_00" name="Button" value="添加" onclick="UsedFieldsSet('add');"> 
          <input type="button" class="system_button_00" name="Button2" value="删除" onclick="UsedFieldsSet('del');"> 
        </td>
        <td rowspan="2" valign="top" width="180"><select name="tempUsedFields" size="4" id="tempUsedFields" style="width:100%;" ondblclick="alert(UsedFields.value);">
          </select></td>
      </tr>
      <tr> 
      ...
      <tr> 
        <td height="6" colspan="3" align="center"> <input type=hidden id=blocktype name=blocktype value="3"> 
          <input type="hidden" name="blockfolder" id="blockfolder" value="<%= request.QueryString("src") %>"> 
          <input name="OutLink" type="hidden" id="OutLink" value=""> <input type=submit class="system_button_00" value="新建模块" onclick="doc.all.blockform.onsubmit=function(){return FromCheck()};">
          　　 
          <input type=button class="system_button_00" value="取消创建" onclick="javascript:window.close();"> 
        </td>
      </tr>
      <tr>
        <td height="6" colspan="3" color="red"><font color="red">说明:请使用于非网站首页的其他页面!</font></td>
      </tr>
    </table>
</form>		
<script language=javascript>
