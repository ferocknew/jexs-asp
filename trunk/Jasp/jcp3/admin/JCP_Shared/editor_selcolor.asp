<% 
dim oldcolor
if request.QueryString("oldcolor")<>"" then
	oldcolor=request.QueryString("oldcolor")
else
	oldcolor="FFFFFF"
end if
%>
...
<CENTER>
    <TABLE border=0 cellPadding=0 cellSpacing=10>
      <TBODY>
        <TR> 
          <TD align=middle rowSpan=3>选中色彩 
            <TABLE border=1 cellPadding=0 cellSpacing=0 height=30 id=ShowColor width=40 bgColor="#<%= oldcolor %>">
              <TBODY>
                <TR> 
                  <TD></TD>
                </TR>
          ...
          <TD rowSpan=3>基色: <SPAN id=RGB></SPAN><BR>
            亮度: <SPAN id=GRAY>120</SPAN><BR>
            代码: 
            <INPUT id=SelColor size=7 value="#<%= oldcolor %>"></TD>
          <TD><BUTTON id=Ok type=submit>确定</BUTTON></TD>
        </TR>
		<TR><TD><BUTTON onclick='window.returnValue="blank";window.close();'>置空</BUTTON></TD></TR>
        <TR>
