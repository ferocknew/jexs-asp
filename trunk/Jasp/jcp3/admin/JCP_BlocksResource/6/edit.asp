...
if not rs.eof then
	...
	if ubound(tproperty)=11 then
		...
		response.write "var strs03=""" & replace(replace(replace(tproperty(11),"<","&lt;"),"""","&quot;"),chr(13)&chr(10),"\n") & """;" & vbcrlf
		response.write "doc.getElementById('blockpageinfo').value=strs03.replace(/&quot;/gi,'""').replace(/&lt;/gi,'\<');" & vbcrlf
	end if
end if
rs.close
%>
