...
Jasp.extend({
	...
	//string转对象的根方法
	parse:function(data){
		return new Jasp.parsefn(data);
	},
	// 输出js
	js: function(words, RType){
        if (RType == 1) {
		var words="<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" \/><"+"script>" + words + "<\/script>";
            //Jasp.output();
			return this;
        }
        else {
...
//转换对象
Jasp.parsefn=function(data){
	this.Jsondata=JSON.parse(data);
};
Jasp.extend(Jasp.parsefn.prototype,{
	output: function(type){
        Jasp.output(this.Jsondata, type);
        return this;
    }
});
// 转换xml
