...
class CheckCodeClass
	Public Noisy, Count, Width, Height, Angle, Offset, Border, Red, Green, Blue
	...
	Public Sub SetDraw(pIndex, pNumber)
		...
		DotData(9) = Array(100,30,100,20,70,1,30,1,1,20,1,30,1,70,1,80,30,100,70,100,100,80,100,70,100,30)
	...
	'说明：将验证码数据流写入缓存
	'传参：checkcodeid－验证码ID
	'返回：无
	Public Sub Create(checkcodeid)
		Session("CheckCode_" & checkcodeid) = NewCode()
		...
		Response.BinaryWrite ChrB(Red) & ChrB(Green) & ChrB(Blue)
		' 图象标识符
	...
	'说明：将验证码数据流写入缓存
	'传参：usercheckcode－用户输入的验证码
	'　　　checkcodeid－验证码ID
	'返回：T/F
	public function Check(usercheckcode,checkcodeid)
		if Session("CheckCode_" & checkcodeid)=usercheckcode then
			Check=true
		else
			Check=false
		end if
